...
const SUGGESTIONS = [
  "New duplex build for the Chen family at 14 Bellara St, Nundah, starting next month.",
  "Received a delivery of 12 roof trusses from Bunnings for the Chen job.",
  "Log 8 hours for Alan on the Wattlebird St project today.",
  "Any compliance issues across my projects right now?",
];
...
export function AskDashboardChat() {
  ...
  async function ask(question: string) {
    ...
    try {
      ...
      // A new project/RFI/variation/etc. may have just been created —
      // refresh so dashboard stats and widgets pick it up.
      if (data.action) router.refresh();
    } catch {
  ...
  return (
    <Card className="mt-6 animate-slide-up overflow-hidden p-0">
      <div className="p-5 pb-0">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Create a new project, or log something against an existing one — a delivery, hours worked, an RFI,
          a variation — all without opening it first.
        </p>
      </div>

      {messages.length === 0 && (
        <div className="p-5">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Try:</p>
      ...
      {messages.length > 0 && (
        <div ref={scrollRef} className="max-h-[28rem] overflow-y-auto p-4 pt-3 space-y-3">
          {messages.map((m, i) => (
      ...
      >
        <input
          ...
          placeholder="Create a project, or log something against one…"
